...
def add_simple_interactions():
    interactions = [
        ...
        (r'не работает мыш(ка|ь)', 'В ноуте барахлит один usb-разъём (правый ближний), возможно, дело в этом?'),
        (r'не работает (клава|клавиатура)', 'Клавиатура в админском ноуте сломана. Подключи внешнюю.'),
        (r'не (работает|подключается) (VR|ВР|шлем)', 'Почитай: https://wiki.admin.kocherga.club/VR'),
    ]
    def gen_cb(response):
        def f(msg, *args):
            return response

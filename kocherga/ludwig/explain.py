...
def explain(query, quiet=False, try_harder=False):
    if query.lower() == 'лфт':
        return {
            'text': 'Это такая зелёная книга в ГЭБ. Я её написал!'
        }

    title = search_wiki(query)
    if title:
        prefixes = ['Нашёл', 'Вот статья на вики', 'Вот тут что-то есть', 'Статья на вики', 'На вики есть статья']
        prefix = prefixes[random.randint(0, len(prefixes) - 1)]
        return {
            'text': '{}: {}'.format(prefix, wiki_link(title)),
        }
    ...
    if try_harder:
        ...
        if titles:
            return {
                'text': 'Я не нашёл статью на вики с заголовком "{}", но нашёл несколько упоминаний:'.format(query),
                'attachments': [
                    {
                        'title': wiki_link(title)
                    }
                    for title in titles
                ]
            }
    ...
    return {
        'text': 'Ничего не знаю про "{}".'.format(query),
    }

...
INFLECTED_WEEKDAY_NAMES = ['понедельник', 'вторник', 'среду', 'четверг', 'пятницу', 'субботу', 'воскресенье']
INFLECTED_MONTH_NAMES = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря']
...
def get_current_watchman_or_complain(message):
    ...
    if watchman == 'Ночь':
        ...
        last = kocherga.team.find_member_by_short_name(kocherga.watchmen.last_watchman())
        nearest = kocherga.team.find_member_by_short_name(kocherga.watchmen.nearest_watchman())

        message.reply(f'Сейчас ночь. Вечером админил(а) {last.short_name}, а утром будет {nearest.short_name}.')
        return

    member = kocherga.team.find_member_by_short_name(watchman)
    if not member:
        raise ErrorResponse(f'Админит {watchman}, но у меня не получилось найти этого человека в базе сотрудников.')

    if not member.slack_id:
        raise ErrorResponse(f'Админит {watchman}, {member.full_name}, но я не знаю, кто это в слаке.')
...
@bot.listen_to(r'кто\s+админил\s+в[о]?\s+(понедельник|вторник|среду|четверг|пятницу|субботу|воскресенье)\?')
...
@bot.listen_to(r'кто\s+(?:админит|будет\s+админить)\s+в[о]?\s+(понедельник|вторник|среду|четверг|пятницу|субботу|воскресенье)\?')

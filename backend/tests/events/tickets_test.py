...
def test_send_reminders(frozen_time, user1, user2, tomorrow_event, future_event):
    ...
    assert len(mail.outbox) == initial_reminders  # no reminders - events were just created

    frozen_time.tick(delta=timedelta(days=1))
    Ticket.objects.send_reminders()
    assert len(mail.outbox) == initial_reminders  # no reminders - future events are 2 days ahead

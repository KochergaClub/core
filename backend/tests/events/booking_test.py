...
from kocherga.events.booking import (
    Booking,
    add_booking,
    delete_booking,
    day_bookings,
...
class TestBooking:
    def test_constructor(self):
        booking = Booking(
            datetime.now(TZ), datetime.now(TZ) + timedelta(hours=1), 'гэб', 5
        )
    ...
    def test_constructor_wrong_room(self):
        with pytest.raises(PublicError, match='Unknown room'):
            Booking(
                datetime.now(TZ), datetime.now(TZ) + timedelta(hours=1), 'блаблабла', 5
            )
...
class TestDayBookings:
    ...
    def test_returns_all(self):
        ...
        add_booking(
            d.strftime('%Y-%m-%d'), 'гэб', 3, '12:00', '12:30', 'first@example.com'
        )
        add_booking(
            d.strftime('%Y-%m-%d'), 'гэб', 3, '13:00', '13:30', 'second@example.com'
        )
        add_booking(
...
class TestCheckAvailability:
    def test_no_params(self):
        with pytest.raises(TypeError):
            check_availability()  # type: ignore

    def test_date_comparison(self):
        with pytest.raises(
            PublicError, match='Starting date and ending date should be equal.'
        ):
            check_availability(
                datetime.now(TZ), datetime.now(TZ) + timedelta(days=1), 'гэб'
            )

    def test_normal(self):
        result = check_availability(
            datetime.now(TZ).replace(hour=18), datetime.now(TZ).replace(hour=19), 'гэб'
        )
...
class TestAddBooking:
    ...
    def test_validate_people(self):
        with pytest.raises(PublicError, match='Invalid number of people'):
            add_booking(
                '2017-01-01', 'гэб', '-5', '12:00', '12:30', 'somebody@example.com'
            )
    ...
    def test_validate_zero_people(self):
        with pytest.raises(PublicError, match='Zero people walk into a bar'):
            add_booking(
                '2017-01-01', 'гэб', 0, '12:00', '12:30', 'somebody@example.com'
            )

    def test_validate_room(self):
        with pytest.raises(PublicError, match='Unknown room'):
            add_booking(
                '2017-01-01', 'blah', 3, '12:00', '12:30', 'somebody@example.com'
            )

    def test_past_date(self):
        with pytest.raises(PublicError, match='The past is already gone'):
            add_booking(
                '2017-01-01', 'гэб', 3, '12:00', '12:30', 'somebody@example.com'
            )

    def test_too_far_date(self):
        with pytest.raises(PublicError, match='too far'):
            add_booking(
                '2025-01-01', 'гэб', 3, '12:00', '12:30', 'somebody@example.com'
            )
    ...
    def test_add_duplicate(self):
        ...
        with pytest.raises(
            PublicError, match='This room is not available at that time'
        ):
            add_booking(
                (datetime.now(TZ) + timedelta(days=1)).strftime('%Y-%m-%d'),
                'гэб',
...
def test_google_export(test_google_calendar_id):
    ...
    google_calendar = GoogleCalendar.objects.create(
        calendar_id=test_google_calendar_id, public_only=False,
    )

...
class TestList:
    def test_anon(self, client, public_event):
        res = client.get(f'/api/event/{public_event.uuid}/tickets')
    ...
    def test_list_by_admin(self, admin_client, public_event):
        res = admin_client.get(f'/api/event/{public_event.uuid}/tickets')
...
class TestMyCreate:
    def test_create_anon(self, client, public_event):
        res = client.post(f'/api/event/{public_event.uuid}/tickets/my')
        assert res.status_code == 403
...
class TestMyGet:
    def test_my_anon(self, client, public_event):
        res = client.get(f'/api/event/{public_event.uuid}/tickets/my')
        assert res.status_code == 403
...
class TestMyDelete:
    def test_my_delete(self, client, public_event, user1):
        ...
        res = client.delete(f'/api/event/{public_event.uuid}/tickets/my')
